import type { Cerebro, Veredicto } from "../nucleo/cerebro";

export interface OpcionesFalso {
  respuesta?: (entrada: string) => string | null;
...
export function cerebroFalso(opciones: OpcionesFalso = {}): Cerebro {
  return {
    ...
    async responder(_sistema, entrada) {
      if (opciones.respuesta) return opciones.respuesta(entrada);
      ...
      if (entrada.includes('{"tema":')) {
        ...
        return JSON.stringify({ tema, titulo: `Artículo de prueba sobre ${tema}`, contenido: `Lo que se aceptó en el hilo, resumido por un cerebro falso ${referencia}.` });
      }
      return `Respuesta de prueba. Recibí: ${entrada.slice(0, 80)}`;
    },
