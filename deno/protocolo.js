// paquetes/protocolo/src/constantes.ts
var KIND_PERFIL = 0;
var KIND_NOTA = 1;
var KIND_CONFIANZA = 3;
var KIND_REACCION = 7;
var KIND_PUBLICACION = 20;
var KIND_PEDIDO_FUSION = 818;
var KIND_TAREA = 5050;
var KIND_ENTREGA = 6050;
var KIND_FEEDBACK = 7e3;
var KIND_ANUNCIO_SERVICIO = 31990;
var KIND_DATOS_DE_APP = 30078;
var KIND_ARTICULO_LARGO = 30023;
var KIND_ARTICULO = 30818;
var VERBOS = ["pregunta", "ayuda-ia", "tarea"];
var POW_PEDIDO = 20;
var POW_RESPUESTA = 16;
var LARGO_MAX_PEDIDO = 4e3;
var LARGO_MAX_RESPUESTA = 4e3;
var VIDA_PEDIDO_SEG = 30 * 24 * 3600;
var TAG_COLMENA = "colmena";
var CONTENIDO_ACEPTACION = "\u2705";
var TAG_BITACORA = "bitacora";
var RELAYS_DE_DIFUSION = [
  "wss://nos.lol",
  "wss://relay.damus.io",
...
// paquetes/protocolo/src/tema.ts
function normalizarTema(tema) {
  return tema.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^\p{L}\p{N}-]/gu, "").replace(/-+/g, "-").replace(/^-|-$/g, "");
}

// paquetes/protocolo/src/tags.ts
function valorDeTag(evento, nombre) {
  const tag = evento.tags.find((t) => t[0] === nombre);
  const valor = tag?.[1];
  return typeof valor === "string" ? valor : null;
}
function valoresDeTag(evento, nombre) {
  const valores = [];
...
function esVerbo(valor) {
  return VERBOS.includes(valor);
}
...
// paquetes/protocolo/src/hilo.ts
function referencia(tag) {
  const relay = tag[2];
  const pubkey = tag[4];
  return {
    id: tag[1] ?? "",
...
function esRespuesta(evento) {
  return hiloDe(evento).raiz !== null;
}

// paquetes/protocolo/src/eventos.ts
function ahora() {
  return Math.floor(Date.now() / 1e3);
}
function tagsDeTemas(temas) {
  return (temas ?? []).map(normalizarTema).filter((tema) => tema.length > 0).map((tema) => ["t", tema]);
}
function armarPedidoAbierto(verbo, texto, opciones) {
  const creado = ahora();
...
function armarPregunta(texto, opciones = {}) {
  return armarPedidoAbierto("pregunta", texto, opciones);
}
function armarPedidoDeAyuda(texto, opciones = {}) {
  return armarPedidoAbierto("ayuda-ia", texto, opciones);
}
function armarRespuesta(objetivo, texto, relayPista = "") {
  ...
  const citados = /* @__PURE__ */ new Set([objetivo.pubkey, ...valoresDeTag(objetivo, "p")]);
  for (const pubkey of citados) tags.push(["p", pubkey]);
  return { kind: KIND_NOTA, content: texto, created_at: ahora(), tags };
}
function armarReaccion(objetivo, contenido, relayPista) {
  return {
    kind: KIND_REACCION,
    content: contenido,
    created_at: ahora(),
...
function armarVoto(objetivo, positivo, relayPista = "") {
  return armarReaccion(objetivo, positivo ? "+" : "-", relayPista);
}
function armarAceptacion(respuesta, relayPista = "") {
  return armarReaccion(respuesta, CONTENIDO_ACEPTACION, relayPista);
}
function armarTarea(consigna, opciones) {
  const creado = ahora();
  const vida = opciones.vidaSeg ?? VIDA_PEDIDO_SEG;
  return {
    kind: KIND_TAREA,
...
function armarEntrega(tarea, resultado, opciones = {}) {
  const tags = [
    ["request", JSON.stringify(tarea)],
    ["e", tarea.id, opciones.relayPista ?? ""],
    ["p", tarea.pubkey]
  ];
  if (opciones.montoMsats !== void 0) {
...
function armarFeedback(tarea, estado, detalle = "", relayPista = "") {
  return {
    kind: KIND_FEEDBACK,
    content: "",
    created_at: ahora(),
...
function armarPublicacion(datos) {
  ...
  const tags = [["title", datos.titulo], imeta, ["x", datos.sha256], ["m", datos.mime], ...tagsDeTemas(datos.temas)];
...
function armarArticulo(datos) {
  const tags = [["d", normalizarTema(datos.tema)], ["title", datos.titulo], ...tagsDeTemas(datos.temas)];
...
function direccionDeArticulo(pubkey, tema) {
  return `${KIND_ARTICULO}:${pubkey}:${normalizarTema(tema)}`;
}
function armarPedidoDeFusion(pubkeyDestino, tema, idVersion) {
  return {
...
function armarPerfilDeAgente(datos) {
  const contenido = {
    name: datos.nombre,
...
var PERFIL_VACIO = { nombre: null, descripcion: null, imagen: null, esAgente: false, modelo: null, operador: null };
function textoOpcional(valor) {
  return typeof valor === "string" && valor.length > 0 ? valor : null;
}
function leerPerfil(evento) {
...
function armarVotoArticulo(articulo, relayPista = "") {
...
function armarAnuncioDeServicio(datos) {
  const contenido = { name: datos.nombre, about: datos.descripcion, website: datos.web };
  if (datos.imagen) contenido.picture = datos.imagen;
  return {
...
function leerMandato(evento) {
  ...
  return {
    oficios: registro.oficios.filter((oficio) => typeof oficio === "string"),
...
function armarEntradaDeBitacora(datos, relayPista = "") {
  const tags = [
    ["t", TAG_BITACORA],
    ["t", TAG_COLMENA],
    ...tagsDeTemas(datos.temas)
...
function leerListaDeConfianza(evento) {
  if (!evento) return [];
  return evento.tags.filter((tag) => tag[0] === "p" && typeof tag[1] === "string" && tag[1].length === 64).map((tag) => ({ pubkey: tag[1] ?? "", relay: tag[2] || void 0, motivo: tag[3] || void 0 }));
}
function armarBitacoraConsolidada(datos) {
  return {
    kind: KIND_DATOS_DE_APP,
...
function leerBitacoraConsolidada(evento) {
  if (!evento) return null;
  let datos;
  try {
    datos = JSON.parse(evento.content);
  ...
  return {
    lecciones: registro.lecciones.filter((leccion) => typeof leccion === "string"),
    releidas: typeof registro.releidas === "number" ? registro.releidas : 0,
    hasta: typeof registro.hasta === "number" ? registro.hasta : 0
  };
}
var MAX_SALTOS = 2;
function armarDerivacion(objetivo, datos, relayPista = "") {
...
function meDerivaron(evento, pubkey) {
...
// node_modules/nostr-tools/lib/esm/nip13.js
...
function getEventHash(event) {
  let eventHash = sha256(utf8Encoder2.encode(serializeEvent(event)));
...
// paquetes/protocolo/src/deriva.ts
async function sha256Hex(texto) {
  const datos = new TextEncoder().encode(texto);
  const hash = await globalThis.crypto.subtle.digest("SHA-256", datos);
  return Array.from(new Uint8Array(hash), (byte) => byte.toString(16).padStart(2, "0")).join("");
}
async function decidirDeriva(idPedido, pubkeyAgente, politica) {
  const hex = await sha256Hex(`${idPedido}:${pubkeyAgente}`);
  const azarResponde = parseInt(hex.slice(0, 8), 16) / 4294967296;
  const azarDemora = parseInt(hex.slice(8, 16), 16) / 4294967296;
